...
const FAQ = ({ style_3, inner_faq, data = answer_question_data }) => {
  const [faqData, setFaqData] = useState([]);

  useEffect(() => {
    const initialFaqData = data.map((faq, index) => ({
      ...faq,
      showAnswer: index === 0,
    }));
  ...
  return (
    <section
      className={`faq-area faq-padding ${
        style_3 ? "faq-two-padding" : "" || inner_faq ? "inner-faq-padding" : ""
      }`}
    >
      <div className="container">
        <div
          className={`row justify-content-center ${
            inner_faq ? "" : "justify-content-xl-between"
          }`}
        >
          ...
          <div
            className={`${
              inner_faq ? "col-xxl-8 col-xl-9 col-lg-11" : "col-xl-7 col-lg-10"
            }`}
          >
